...
//찜 개수 가져오기
async function find_wishcount(user_id) {
    const conn = await pool.getConnection();
    const row = conn.query(
        "SELECT COUNT(*) AS count FROM wish WHERE user_id = ?",
        [user_id]
    );
    conn.release();
    return row;
}
router.post("/Mypage_wishlist_count", async (req, res) => {
    const { user_id } = req.body;
    const rows = await find_wishcount(user_id);
    count = rows[0].count;
    // BigInt 처리 (toString 또는 Number로 변환)
    if (typeof count === "bigint") {
        count = Number(count); // 또는 count.toString()
...
//찜목록 초기화
router.post("/Mypage_wish_init", async (req, res) => {
    const { user_id } = req.body;
    const conn = await pool.getConnection();
    const rows = await conn.query(
    ...
    conn.release();
    // rows 배열에서 product_id만 추출하여 배열로 반환
...
router.post("/Mypage_Wish", async (req, res) => {
    ...
    if (likedItems === true) {
        //wish리스트 데이터 추가
        const data_result = await conn.query(
